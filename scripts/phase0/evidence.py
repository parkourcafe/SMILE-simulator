...
def render_report(evaluation: Evaluation, *, now: datetime | None = None) -> str:
    ...
    lines = [
        ...
        f"- Face model SHA-256: `{evaluation.run_configuration.get('face_model_sha256', 'unknown')}`",
    ...
    lines.extend(
        [
            ...
            "## Score matrix",
            "",
            "| File | Style | Tooth realism | Face preservation | Boundary blending | Style accuracy | Emotional response | Row average | Notes |",
            "|---|---|---:|---:|---:|---:|---:|---:|---|",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "## Fixed decision rule",
            "",
            "- GO: exactly 10/10 successful generations with one fixed style, overall average at least 3.5, every criterion average at least 2.0, and no recurring identity failure.",
            "- ITERATE: overall average 3.0-3.49, a criterion below 2.0, any failed generation, or non-standard evidence.",
            "- NO-GO: overall average below 3.0 or confirmed recurring identity/face-preservation failure.",
            "",
