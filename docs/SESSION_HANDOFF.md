...
> Updated: 10.07.2026 | 17:26 | Bali | Codex (privacy-safe request observability and Sentry gate prepared)
> Updated: 10.07.2026 | 19:16 | Bali | Codex (request boundaries, safe GET retries, and background correlation prepared)
> Updated: 10.07.2026 | 23:35 | Bali | Codex (0008 verified remotely, publishable key selected, REST/RLS smoke test passed)\n> Для нового чата: прочитай CLAUDE.md (конституция), затем этот файл, затем продолжай с «Следующие шаги».
...
| CI | `.github/workflows/ci.yml` — database migrations/invariants + backend lint/test + production Docker smoke + Flutter analyze/test. PR #22 и повторный run на `main` зелёные. |
...
- **Бэкенд** (`backend/`): FastAPI, ML-пайплайн, современная Supabase JWT/key-поддержка, idempotent leads, retryable hard deletion + 30-day retention job. 120 тестов зелёные локально. Production Railway image запускается non-root, использует dynamic `PORT`, pinned dependency lock и проверенный Face Landmarker; `/health` и dependency-aware `/ready` покрыты CI. Серверный consent receipt создаётся до private upload, quota резервируется атомарно. YooKassa state перепроверяется server-to-server и активирует ровно один pack. Каждый ответ получает безопасный `X-Request-ID`; JSON logs не содержат query/body/IP/raw exceptions. Sentry включается только через DSN с дополнительным PII scrubber, а production без DSN не стартует. Подготовлены лимит JSON body 256 KiB, максимум три повтора только для идемпотентных внешних GET и явная корреляция generation worker с исходным request ID. Remote Railway, Sentry smoke и YooKassa test-shop E2E ещё не выполнены.
...
- **База** (Supabase): numbered migrations `0001`–`0014` подготовлены. `0008` добавляет waitlist/clinic applications + consent metadata, `0009` — photo retention/tombstones, `0010` — lead consent/security/idempotency, `0011` — provisioning `public.users` после Auth OTP, `0012` — photo-processing receipts + Storage RLS, `0013` — atomic quota, `0014` — durable YooKassa intents + one-payment/one-pack activation. На чистом PostgreSQL 16 локально прошли весь migration chain, behavioral SQL assertions, повторная payment activation и две конкурентные reservation-сессии. Удалённо через Supabase-коннектор подтверждены таблицы, RLS и grants для `0008`; `0009`–`0014` по-прежнему нельзя считать применёнными до отдельной проверки. `seed_dev.sql` запрещён для production.
- **Hero-сайт** (`web/index.html`): бренд ZubiLook, RU/EN/UZ переключатель, тёмный кинематографичный, AIDA, 2 AI-видео + 4 AI-фото (AI-визуализация, честные дисклеймеры), формы «Ранний доступ» + «Клиника-партнёр». Premium-pass применён: Cormorant Garamond + Onest, product-first hero, editorial timeline/ledger вместо части card grids, mobile hero rules, локальные медиа в `web/assets/`. Production deploy на Vercel: `https://www.zubilook.com/`. Формы отправляют заявки в Supabase REST с публичным publishable key; smoke test прошёл: обе вставки `201`, отказ без согласия `401`, anon read `401`.
...
- **Backend release gates**: PR #9 (auth/config), #11 (photo retention), #13 (lead security), #14 (Flutter clinic lead path), #15 (Railway image/readiness), #19 (pre-upload consent), #20 (atomic quota), #21 (verified YooKassa) и #22 (privacy-safe observability) слиты. `APP_ENV=production` запрещает mocks, неверную модель, отсутствующие credentials/Sentry, unsafe CORS и отсутствие канала уведомления клиники.
...
1. **Supabase + формы**: `0008` проверена удалённо, публичный REST smoke test пройден и тестовые строки удалены. Остались `0009`–`0014`: применять по порядку после preflight из `SETUP.md`; затем задеплоить PR на Vercel. Реальные пользовательские B2C/B2B заявки проверять после публикации.
